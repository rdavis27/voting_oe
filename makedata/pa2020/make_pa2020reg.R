...
url <- "https://www.pa.gov/content/dam/copapwp-pagov/en/dos/old-website-documents/VoterRegistration_2020_General_Precinct.txt"
datapath <- "data/"
makepath <- "makedata/pa2020/"
makepath_pre <- paste0(makepath,"PA20reg_") #add file prefix
filename <- "20201103__pa__GEreg__precinct.csv"
#END PARAMETERS
...
# oo <- as.data.frame(read_delim(paste0(makepath_pre,"office_code.txt"),delim = '\t')) #UPDATE

#Convert data to long format with 7 required fields
names(dd) <- hh$Field
dd <- dd[dd$County_Code > 0,] #UPDATE (ADD)
